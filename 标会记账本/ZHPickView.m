...
@property(nonatomic,assign)NSDate *limitStartDate; //从某时间开始
@property(nonatomic,assign)NSDate *largeDate; //从某时间结束
@end
...
-(void)setUpStartDatePickerWithstartDatePickerMode:(UIDatePickerMode)datePickerMode{
    UIDatePicker *datePicker=[[UIDatePicker alloc] init];
    datePicker.locale = [[NSLocale alloc] initWithLocaleIdentifier:@"zh_CN"];
    datePicker.datePickerMode = datePickerMode;
//    datePicker.backgroundColor=[UIColor lightGrayColor];
...
-(void)LimitStartDatePickerWithstartDatePickerMode:(UIDatePickerMode)datePickerMode{
    UIDatePicker *datePicker=[[UIDatePicker alloc] init];
    datePicker.locale = [[NSLocale alloc] initWithLocaleIdentifier:@"zh_CN"];
    datePicker.datePickerMode = datePickerMode;
//    datePicker.backgroundColor=[UIColor lightGrayColor];
...
-(void)setUpToolBarTwo{
    _toolbar=[self setDatePickerViewToolbarStyle];
    [self setToolbarWithPickViewFrame];
    [self addSubview:_toolbar];
}

//添加农历 阳历
-(UIToolbar *)setDatePickerViewToolbarStyle{
    UIToolbar *toolbar=[[UIToolbar alloc] init];
    ...
    _segmentControl = [[UISegmentedControl alloc]initWithItems:[NSArray arrayWithObjects:@"农历",@"阳历", nil]];
    [_segmentControl addTarget:self action:@selector(segmentAction:) forControlEvents:UIControlEventTouchUpInside];
    _segmentControl.selectedSegmentIndex = 1;
    UIBarButtonItem *segmentButtonItem =[[UIBarButtonItem alloc]initWithCustomView:_segmentControl];
    
    
    UIBarButtonItem *centerSpaceTwo=[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace target:self action:nil];
    
    
    UIBarButtonItem *right=[[UIBarButtonItem alloc] initWithTitle:@"确定" style:UIBarButtonItemStylePlain target:self action:@selector(doneClick)];
    toolbar.items=@[lefttem,centerSpace,segmentButtonItem,centerSpaceTwo,right];
